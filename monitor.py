...
# ── CONFIG ─────────────────────────────────────────────────────────────────────
TW_BEARER_TOKEN    = os.environ.get("TW_BEARER_TOKEN")          # Twitter API bearer token (optional for historical)
...
# ── LIVE TWEET FETCH ─────────────────────────────────────────────────────────────
def fetch_new_tweets(since_id: int | None) -> list[tweepy.Tweet]:
...
def process_live():
    ...
    if SINCE_ID_FILE.exists():
        try:
            ...
            since_id = None
    tweets = fetch_new_tweets(since_id)
    for tw in tweets:
